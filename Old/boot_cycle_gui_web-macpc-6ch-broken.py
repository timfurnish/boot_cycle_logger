...
def _kill_existing_instances():
    """
    Kill any existing Boot Cycle Logger instances and free the port.
    This ensures clean startup without resource conflicts.
    """
    try:
        if platform.system() in ("Darwin", "Linux"):
            # Kill by port
            p = subprocess.run(["lsof", "-ti", "tcp:5055"], capture_output=True, text=True)
            pids = [pid.strip() for pid in (p.stdout or "").splitlines() if pid.strip()]
            for pid in pids:
                try:
                    subprocess.run(["kill", "-9", pid], check=False)
                    print(f"Killed process on port 5055: PID {pid}")
                except Exception:
                    pass
            
            # Kill by process name
            try:
                subprocess.run(["pkill", "-9", "-f", "boot_cycle_gui_web"], check=False)
                print("Killed any boot_cycle_gui_web processes")
            except Exception:
                pass
                
        elif platform.system() == "Windows":
            # Kill by port
            ps = "(Get-NetTCPConnection -LocalPort 5055 -State Listen -ErrorAction SilentlyContinue).OwningProcess"
        ...
        print(f"Note: Could not kill existing instances: {e}")
        pass
...
SRC        = "0"        # default camera index; will be auto-detected

def _detect_obs_virtual_camera():
    """
    Probe video sources 0-4 and automatically select the one with valid video feed.
    Looks for OBS Virtual Camera or any active video source.
    Returns the index of the first working source, or "1" as fallback.
    """
    print("\n" + "="*60)
    print("AUTO-DETECTING VIDEO SOURCE...")
    print("="*60)
    
    backend = cv2.CAP_AVFOUNDATION if platform.system() == "Darwin" else cv2.CAP_MSMF
    
    for idx in range(5):  # Test sources 0-4
        try:
            print(f"Testing source {idx}...", end=" ")
            ...
            if ok and frame is not None and frame.size > 0:
                ...
                # Check if it's a valid video (not completely black)
                if mean_brightness > 5.0 and w >= 640 and h >= 480:
                    print(f"✅ FOUND! Size: {w}×{h}, Brightness: {mean_brightness:.1f}")
                    cap.release()
                    print(f"\n🎥 Using video source: {idx}")
                    print("="*60 + "\n")
                    return str(idx)
                else:
                    print(f"⚠️  Too dark or small ({w}×{h}, brightness: {mean_brightness:.1f})")
            else:
    ...
    # Fallback
    print("\n⚠️  No valid source found, using default: 1")
    print("="*60 + "\n")
    return "1"

# Auto-detect OBS Virtual Camera on startup
SRC = _detect_obs_virtual_camera()
...
def run_loop():
    ...
    for i in range(20):
        ...
        time.sleep(0.05)  # 50ms between reads
    print("[run_loop] Warmup complete, starting detection...")
...
if __name__ == "__main__":
    print("\n" + "="*60)
    print("BOOT CYCLE LOGGER - STARTING UP")
    print("="*60)
    
    # Kill any existing instances first
    print("\n1. Cleaning up existing instances...")
    ...
    print(f"\n3. Starting server on http://localhost:{PORT}")
    print("="*60 + "\n")
