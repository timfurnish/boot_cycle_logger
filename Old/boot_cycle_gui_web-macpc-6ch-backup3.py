...
def _csv_append(video:int, state:str, elapsed_secs=None, cycle_num=None, bars_dist=None, int_dist=None, event_type:str="state_change"):
    ...
    Columns: Timestamp, Video Channel, Console Serial, Scope ID, State, Elapsed Secs, 
             Cycle Number, Disconnected Match Distance, Connected Match Distance, Event Type
...
def index():
    ...
    .test-indicator{display:inline-block;width:48px;height:48px;border-radius:50%;margin-left:12px;vertical-align:middle;opacity:0;transition:opacity 0.3s ease;position:relative;top:2px}
    .test-indicator.running{background:#10b981;opacity:1;animation:pulse 1.5s ease-in-out infinite}
    @keyframes pulse{0%,100%{opacity:1;box-shadow:0 0 0 0 rgba(16,185,129,0.7)}50%{opacity:0.6;box-shadow:0 0 0 16px rgba(16,185,129,0)}}
      ...
      <div class='left'>
        ...
        <div class='camera-section' style='background:var(--bg);padding:12px;border-radius:8px;margin-bottom:12px;border:1px solid var(--border)'>
          ...
          <div style='display:flex;gap:8px;margin-top:8px'>
            ...
            <button class='secondary' onclick='forceOBSDetection()' style='flex:1'>Force OBS Detection</button>
          </div>
        ...
        <div id='reportModal' class='modal' style='display:none'>
          <div class='modal-content' style='max-width:800px;max-height:80vh;overflow-y:auto'>
            ...
            <div style='display:flex;gap:12px;margin-top:20px'>
              <button onclick='goToCSV()' id='goToCsvBtn' style='flex:1;padding:12px'>Go to CSV</button>
  ...
  async function startTest(){
    try {
      ...
      if (j.ok) {
        console.log('Test started, CSV:', j.csv_path);
        // Show running indicator
        document.getElementById('testIndicator').classList.add('running');
  ...
  async function endTest(){
    try {
      ...
      if (!r.ok) {
        ...
        alert('Error ending test: ' + r.statusText);
        return;
      }
  ...
  async function autoDetectCamera(){
    ...
    try {
      ...
      if (j.ok) {
        ...
        document.getElementById('backend').value = j.backend;
        
        statusEl.style.background = '#064e3b';
        statusEl.style.color = '#10b981';
        statusEl.textContent = `✓ Found OBS Virtual Camera: Source ${j.index}, Backend ${j.backend}`;
        ...
        statusEl.style.background = '#7f1d1d';
        statusEl.style.color = '#ef4444';
        statusEl.textContent = '✗ OBS Virtual Camera not found: ' + (j.error || 'No camera found');
      }
    } catch(e) {
      statusEl.style.background = '#7f1d1d';
      statusEl.style.color = '#ef4444';
    ...
    try {
      ...
      if (j.ok && j.cameras && j.cameras.length > 0) {
        ...
        j.cameras.forEach(cam => {
          ...
          html += `${typeEmoji} Source ${cam.index}: ${cam.type} (${cam.size})<br>`;
        });
        ...
        listEl.textContent = '✗ No cameras found';
      }
    } catch(e) {
      listEl.style.background = '#7f1d1d';
      listEl.style.color = '#ef4444';
...
def identify_camera_type(cap, idx):
    """
    Try to identify if a camera is OBS Virtual Camera based on its properties.
    Returns: "obs_virtual", "facetime", "other", or "unknown"
    """
    try:
        # Get camera properties that might help identify it
        width = cap.get(cv2.CAP_PROP_FRAME_WIDTH)
        height = cap.get(cv2.CAP_PROP_FRAME_HEIGHT)
        fps = cap.get(cv2.CAP_PROP_FPS)
        
        # Try to get camera name/identifier (this might not work on all systems)
        try:
            # Some backends support getting device names
            backend = cap.getBackendName()
        except:
            backend = "unknown"
        
        # More sophisticated OBS Virtual Camera detection
        # OBS Virtual Camera typically has these characteristics:
        # - Usually 1920x1080 or 1280x720 (but not always)
        # - 30 FPS (but not always)
        # - Often at higher indices (1, 2, 3, etc.)
        # - On Windows, typically uses DSHOW backend when available
        ...
        if platform.system() == "Darwin":  # macOS
            ...
            elif idx in [1, 2] and is_standard_resolution and is_standard_fps:
                return "obs_virtual"  # Likely OBS Virtual Camera
            elif is_small_resolution:
                return "other"  # Small cameras are usually not OBS
            else:
                return "other"
        else:  # Windows/Linux
            # On Windows, be more conservative about OBS detection
            # Only mark as obs_virtual if it's NOT at index 0 AND has standard resolution
            if idx == 0:
                # Index 0 is usually the built-in laptop camera
                if is_small_resolution:
                    return "other"  # Small built-in camera
                else:
                    return "other"  # Regular built-in camera (not OBS)
            elif is_small_resolution:
                return "other"  # Small cameras are usually not OBS
            elif idx >= 1 and is_standard_resolution and is_standard_fps:
                return "obs_virtual"  # Higher index with standard resolution could be OBS
            else:
                return "other"
...
def auto_detect_camera():
    ...
    if platform.system() == "Windows":
        # Windows: Try OBS Virtual Camera by name first, then indices
        ...
        # Try different backends for OBS
        obs_backends = [
            ("DSHOW", cv2.CAP_DSHOW),
            ("MSMF", cv2.CAP_MSMF),
            ("ANY", cv2.CAP_ANY)
        ...
        # Windows: Try DSHOW first (better for OBS), then MSMF
        backends = [
            ("DSHOW", cv2.CAP_DSHOW),
            ("MSMF", cv2.CAP_MSMF)
        ]
        
        # Try indices 0-10 with each backend
        for backend_name, backend_code in backends:
            for idx in range(11):
                try:
                    print(f"[Camera Detection] Trying index {idx} with {backend_name}...")
                    ...
                    if cap.isOpened():
                        ...
                        if ret and frame is not None:
                            h, w = frame.shape[:2]
                            cap.release()
                            print(f"[Camera Detection] ✓ Found working camera: index={idx}, backend={backend_name}, size={w}x{h}")
        ...
        print("[Camera Detection] ⚠ No cameras found, using defaults (0, DSHOW)")
        ...
        # Check more camera indices (0-10) to find all available cameras
        for idx in range(11):
            try:
        ...
        # Prioritize OBS Virtual Camera, but be more conservative on Windows
        ...
        if obs_cameras:
            # Use the first OBS Virtual Camera found
...
def auto_detect_camera_endpoint():
    ...
    try:
        ...
        return jsonify(ok=True, index=idx, backend=backend_name)
    except Exception as e:
...
def list_cameras():
    ...
    try:
        cameras = []
        backend_code = cv2.CAP_AVFOUNDATION if platform.system() == "Darwin" else (cv2.CAP_DSHOW if platform.system() == "Windows" else cv2.CAP_V4L2)
...
def start_detection():
    ...
    with mon.lock:
        ...
        try:
            ...
            f.close()
            print(f"[Start Test] Created CSV with headers: {mon.csv_path}")
        except Exception as e:
...
@app.post("/end_test")
def end_test():
    """End the test, generate report, and append to CSV."""
    with mon.lock:
        mon.detection_active = False
...
@app.post("/reveal_csv")
def reveal_csv():
    """Open the file browser and highlight the most recent CSV file."""
    try:
        ...
        success = _reveal_in_file_browser(csv_path)
        return jsonify(ok=True, success=success, csv_path=csv_path)
    except Exception as e:
...
if __name__ == "__main__":
    # Auto-detect camera on startup for better OBS Virtual Camera support
    try:
        ...
        print(f"[Startup] ✓ Camera ready: Source={detected_idx}, Backend={detected_backend_name}")
    except Exception as e:
        print(f"[Startup] ⚠ Auto-detection failed: {e}, using defaults")
