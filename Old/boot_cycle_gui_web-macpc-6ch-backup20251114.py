...
def run_loop():
    ...
    def _open_with_backend(_src, _backend):
        ...
        # Validate camera stability: read 5 frames and check they're similar
        # This prevents flickering between multiple cameras at the same index
        if cap and cap.isOpened():
            print(f"[Camera Validation] Testing stability of Source {idx}...")
            frame_means = []
            frame_shapes = []
            ...
            for i in range(5):
                ...
                if ret and frame is not None:
                    ...
                    print(f"[Camera Validation] ✗ Failed to read frame {i+1}/5")
    ...
    # Some Windows systems without admin rights have camera enumeration issues
    # This ensures the camera handle stays locked to the initial device
    try:
        if platform.system() == "Windows":
    ...
    try:
        ...
        if platform.system() == "Darwin":  # macOS
            ...
            if idx == 0 and is_small_resolution:
                return "other"  # Built-in laptop camera
            elif is_small_resolution:
                return "other"  # Small cameras
            elif is_standard_resolution and is_standard_fps:
                # Could be USB capture device or OBS Virtual Camera
                # Be conservative - don't assume it's OBS
                return "usb_capture"  # More likely to be USB capture device
            else:
                return "other"
...
def auto_detect_camera():
    ...
    if platform.system() == "Windows":
        ...
        # Try indices 0-20 with each backend (expanded range for systems with many devices)
        ...
        # Check more camera indices (0-30) to find all available cameras (expanded for systems with many devices)
        for idx in range(31):
            try:
