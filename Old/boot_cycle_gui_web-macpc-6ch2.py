...
def open_csv(path):
    ...
    w = csv.writer(f)
    if f.tell()==0:
        # Enhanced column headers with equipment and timing info
        w.writerow([
            "Timestamp",
            "Video Channel",
            "Console Serial",
...
def _generate_test_report():
    ...
    and identification of incomplete cycles.
    ...
    report_lines.append(f"CSV File: {os.path.basename(mon.csv_path)}")
    report_lines.append("")
    ...
    report_path = os.path.join(LOG_ROOT, report_filename)
    try:
        with open(report_path, "w") as f:
            f.write(report_text)
    except Exception as e:
        print(f"[Report] Error saving: {e}")
    ...
    <div class='wrap'>
      ...
      <div class='left'>
        ...
        <div class='row' style='display:flex;gap:8px;flex-wrap:wrap'>
          <button onclick='start()' id='startBtn'>Start Test</button>
          <button class='secondary' onclick='stop()' id='stopBtn'>Stop Test</button>
...
def stop():
    ...
    # Generate report and reveal files
    try:
        report_text, report_path = _generate_test_report()
        print("\n" + report_text)  # Print to console
        
        # Reveal CSV in file browser
        _reveal_in_file_browser(mon.csv_path)
        
        return jsonify(ok=True, report=report_text, report_path=report_path, csv_path=mon.csv_path)
    except Exception as e:
        print(f"[Stop] Error generating report: {e}")
