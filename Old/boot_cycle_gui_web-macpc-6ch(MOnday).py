...
def run_loop():
    ...
    def _open_with_backend(_src, _backend):
        ...
        if isinstance(_src, str) and not _src.isdigit():
            # Named devices (e.g., "OBS Virtual Camera") behave better when we
            # force the backend that detected them. Try that first, then fall back
            # to OpenCV's default resolver so we always return *something*.
            try:
                cap = cv2.VideoCapture(_src, _backend)
        ...
        # Windows/OBS Virtual Camera: Set properties BEFORE reading frames
        # This is critical for OBS Virtual Camera on Windows
        if cap and cap.isOpened() and platform.system() == "Windows":
            try:
                # Set buffer to 1 immediately for faster response
                cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)
                # Set resolution early (OBS Virtual Camera works better with explicit resolution)
                cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1920)
                cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 1080)
                # Small delay to let camera initialize
                time.sleep(0.1)
            except Exception as e:
                print(f"[Camera Validation] Warning setting properties: {e}")
        ...
        # Windows: Skip validation to prevent hanging - just verify camera opens
        if cap and cap.isOpened():
            if platform.system() == "Windows":
                # Windows: Just verify we can read ONE frame, skip full validation
                # This prevents hanging on problematic cameras
                print(f"[Camera Validation] Windows: Quick validation of Source {idx} (1 frame)...")
                try:
                    ret, frame = cap.read()
                    if ret and frame is not None:
                        print(f"[Camera Validation] ✓ Camera opened and can read frames")
                    else:
                        print(f"[Camera Validation] ⚠ Camera opened but cannot read frames yet")
                except Exception as e:
                    print(f"[Camera Validation] ⚠ Error reading frame: {e}")
                # Return cap anyway - let run_loop handle further validation
                return cap
            else:
                # macOS/Linux: Full validation
                ...
                for i in range(num_frames):
                    ...
                    if ret and frame is not None:
                        ...
                        if i < num_frames - 1:
                            delay = 0.05 if platform.system() == "Darwin" else 0.02
                            time.sleep(delay)
                    else:
                ...
                if len(frame_means) >= 3:
                    ...
                    if mean_variance > 50:
                        print(f"[Camera Validation] ⚠️  HIGH VARIANCE ({mean_variance:.1f}) - Camera may be unstable!")
    ...
    if use_stream:
        ...
        print(f"[run_loop] Opening camera: Source={mon.src}, Backend={mon.backend_name} (code={backend})")
        with mon.lock:
            mon.status = "opening_camera"
        cap = _open_with_backend(mon.src, backend)
        if (not cap) or (not cap.isOpened()):
            print(f"[run_loop] ⚠ Failed to open with {mon.backend_name}, trying alternate backend...")
            if platform.system() == "Windows":
                alt_backend = cv2.CAP_DSHOW if backend == cv2.CAP_MSMF else cv2.CAP_MSMF
                alt_backend_name = "DSHOW" if alt_backend == cv2.CAP_DSHOW else "MSMF"
                print(f"[run_loop] Trying alternate backend: {alt_backend_name}")
                try:
                    if cap: cap.release()
                except Exception:
                    pass
                ...
                if cap and cap.isOpened():
                    print(f"[run_loop] ✓ Successfully opened with alternate backend: {alt_backend_name}")
                    # Update backend name to match what actually worked
                    with mon.lock:
                        mon.backend_name = alt_backend_name
            elif platform.system() == "Darwin":
    ...
    # Windows/OBS Virtual Camera: Set resolution explicitly and verify
    try:
        ...
        # Windows: Verify resolution was set (OBS Virtual Camera sometimes needs this)
        if platform.system() == "Windows":
            time.sleep(0.1)  # Small delay for property to take effect
            actual_w = cap.get(cv2.CAP_PROP_FRAME_WIDTH)
            actual_h = cap.get(cv2.CAP_PROP_FRAME_HEIGHT)
            if abs(actual_w - w) > 10 or abs(actual_h - h) > 10:
                print(f"[run_loop] ⚠️  Resolution mismatch: requested {w}x{h}, got {int(actual_w)}x{int(actual_h)}")
                # Try setting again
                cap.set(cv2.CAP_PROP_FRAME_WIDTH,  w)
                cap.set(cv2.CAP_PROP_FRAME_HEIGHT, h)
                time.sleep(0.1)
            else:
                print(f"[run_loop] ✓ Resolution set to {int(actual_w)}x{int(actual_h)}")
    except Exception as e:
        print(f"[run_loop] ⚠️  Error setting resolution: {e}")
    if not cap.isOpened():
        with mon.lock:
    ...
    with mon.lock:
        mon.status = "testing_camera"
    
    # Windows/OBS Virtual Camera: Try multiple times with delays (but fewer attempts)
    ok_probe = False
    probe_frame = None
    max_read_attempts = 3 if platform.system() == "Windows" else 1  # Reduced from 5 to 3
    ...
    for attempt in range(max_read_attempts):
        ...
        if attempt < max_read_attempts - 1:
            print(f"[run_loop] Frame read attempt {attempt + 1} failed, retrying...")
    ...
    if ok_probe and probe_frame is not None:
        ...
        # Check if this is OBS Virtual Camera and if it's black (not started)
        # Note: Device name lookup removed to avoid antivirus triggers
        # We rely on resolution-based detection instead
        if platform.system() == "Windows":
            # Simple check: if source is 1, 2, or 3 and high-res, likely OBS Virtual Camera
            src_idx = mon.src if isinstance(mon.src, int) else None
            is_obs = src_idx in [1, 2, 3]  # OBS Virtual Camera typically at these indices
            
            if is_obs and probe_mean < 10:
                print(f"[run_loop] ⚠️  WARNING: OBS Virtual Camera (likely) detected but frame is BLACK (mean={probe_mean:.1f})")
                print(f"[run_loop] ⚠️  OBS Virtual Camera is not started or has no active source!")
                print(f"[run_loop] ⚠️  Please:")
                print(f"[run_loop] ⚠️    1. Open OBS Studio")
                print(f"[run_loop] ⚠️    2. Click 'Start Virtual Camera' button")
                print(f"[run_loop] ⚠️    3. Make sure you have an active source/scene in OBS")
            elif is_obs:
                print(f"[run_loop] ✓ OBS Virtual Camera is active and streaming (mean={probe_mean:.1f})")
    else:
        print(f"[run_loop] ⚠️  Initial frame read failed after {max_read_attempts} attempts: ok={ok_probe}, frame={'None' if probe_frame is None else 'valid'}")
    ...
    with mon.lock:
        ...
        print(f"[run_loop] ✓ Video feed ready: status={mon.status}, video_running={mon.video_running}")
...
def index():
    ...
    <div id='equipModal' class='modal'>
      <div class='modal-content'>
        ...
        <button id='saveEquipmentBtn' style='width:100%;padding:14px'>Save and Continue</button>
      </div>
    </div>
    ...
    <script>
  // Define all functions at the top level to make them globally accessible
  // This ensures onclick handlers can find them
  
  // Define hideEquipModal early so it's available everywhere
  function hideEquipModal(){
    const modal = document.getElementById('equipModal');
    if (!modal) return;
    
    // Remove show class and force display none
    modal.classList.remove('show');
    modal.style.display = 'none';
    modal.style.setProperty('display', 'none', 'important');
    
    // Backup: use visibility and opacity
    setTimeout(function() {
  ...
  // Forward declarations - define function names immediately so onclick handlers can find them
  // Functions will be defined later in the script
  window.startTest = function() { console.error('startTest not yet loaded'); };
  window.endTest = function() { console.error('endTest not yet loaded'); };
  window.showEquipModal = function() { console.error('showEquipModal not yet loaded'); };
  window.probe = function() { console.error('probe not yet loaded'); };
  window.resetTallies = function() { console.error('resetTallies not yet loaded'); };
  window.connectCamera = function() { console.error('connectCamera not yet loaded'); };
  window.autoDetectCamera = function() { console.error('autoDetectCamera not yet loaded'); };
  window.listAvailableCameras = function() { console.error('listAvailableCameras not yet loaded'); };
  window.resetCameraSystem = function() { console.error('resetCameraSystem not yet loaded'); };
  window.getSystemCameras = function() { console.error('getSystemCameras not yet loaded'); };
  window.forceRestartApp = function() { console.error('forceRestartApp not yet loaded'); };
  window.showRestartInstructions = function() { console.error('showRestartInstructions not yet loaded'); };
  window.goToCSV = function() { console.error('goToCSV not yet loaded'); };
  window.closeReportModal = function() { console.error('closeReportModal not yet loaded'); };
  window.forceOBSDetection = function() { console.error('forceOBSDetection not yet loaded'); };
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Close equipment modal when clicking outside (on backdrop)
    ...
    // Attach event listener to Save and Continue button
    ...
    function attachSaveButton() {
      ...
      if (saveEquipmentBtn) {
        ...
        const newBtn = saveEquipmentBtn.cloneNode(true);
        saveEquipmentBtn.parentNode.replaceChild(newBtn, saveEquipmentBtn);
        
        // Attach new listener
        newBtn.addEventListener('click', function(e) {
          e.preventDefault();
          e.stopPropagation();
          if (window.saveEquipmentAndContinue) {
            window.saveEquipmentAndContinue();
          } else {
            hideEquipModal();
          }
        });
        
        // Also attach as onclick as backup
        ...
        console.warn('Save Equipment button not found, will retry...');
        return false;
      }
    }
    ...
    if (!attachSaveButton()) {
      ...
      setTimeout(function() {
        if (!attachSaveButton()) {
          // Last retry
          setTimeout(attachSaveButton, 500);
        }
      }, 100);
    }
  });
  ...
  // Define startTest and make it globally accessible
  async function startTest(){
    try {
      ...
      if (j.ok) {
        ...
        // Show helpful message if camera isn't connected
        const errorMsg = j.error || 'Unknown error';
        if (errorMsg.includes('Video') || errorMsg.includes('camera') || errorMsg.includes('source')) {
          alert('Camera not connected!\n\n' +
                'CSV file has been created and logging is ready.\n\n' +
                'To connect camera:\n' +
                '1. Make sure OBS Studio is running\n' +
                '2. Click "Start Virtual Camera" in OBS Studio\n' +
                '3. Click "Auto-Detect OBS" or "Connect to Camera" button\n' +
                '4. Once connected, logging will start automatically');
        } else {
  ...
  // Define endTest and make it globally accessible
  ...
  window.forceOBSDetection = forceOBSDetection;
  ...
  async function connectCamera(){
    ...
    try {
      ...
      if (j.ok) {
        // Poll status to see when camera is actually ready
        let attempts = 0;
        ...
        const checkStatus = async () => {
          try {
            ...
            if (statusJ.video_running && statusJ.status === 'video_ready') {
              statusEl.style.background = '#064e3b';
              statusEl.style.color = '#10b981';
            ...
            } else if (statusJ.status && statusJ.status.startsWith('error')) {
              statusEl.style.background = '#7f1d1d';
              statusEl.style.color = '#ef4444';
              statusEl.textContent = '✗ Connection failed: ' + statusJ.status;
            } else if (attempts < maxAttempts) {
              // Update status message based on current state
              const statusMsg = {
                'video_starting': 'Starting video feed...',
                'opening_camera': 'Opening camera...',
                'testing_camera': 'Testing camera...'
              }[statusJ.status] || 'Connecting to camera...';
              ...
              statusEl.style.background = '#7f1d1d';
              statusEl.style.color = '#ef4444';
              statusEl.textContent = '✗ Connection timeout - camera may not be responding';
            }
          } catch(e) {
  ...
  async function autoDetectCamera(){
    ...
    statusEl.style.background = '#1e293b';
    statusEl.style.color = '#94a3b8';
    statusEl.textContent = '🔍 Auto-detecting OBS Virtual Camera...';
  ...
  window.autoDetectCamera = autoDetectCamera;
  ...
  async function listAvailableCameras(){
    ...
    try {
      ...
      if (j.ok && j.cameras && j.cameras.length > 0) {
        let html = '<strong>Available Cameras:</strong><br><br>';
        j.cameras.forEach(cam => {
          ...
          const deviceName = cam.name ? `"${cam.name}"` : '';
          const statusIcon = cam.is_black ? '⚫' : '🟢';
          const statusText = cam.is_black ? ' (BLACK - not streaming)' : ' (active)';
          const meanText = cam.frame_mean !== undefined ? ` mean=${cam.frame_mean}` : '';
          
          html += `${typeEmoji} ${statusIcon} Source ${cam.index} (${backend}): ${cam.type}${deviceName ? ' ' + deviceName : ''} (${cam.size})${statusText}${meanText}<br>`;
        });
  ...
  window.listAvailableCameras = listAvailableCameras;
  ...
  window.getSystemCameras = getSystemCameras;
  ...
  window.resetCameraSystem = resetCameraSystem;
  ...
  window.forceRestartApp = forceRestartApp;
  ...
  window.showRestartInstructions = showRestartInstructions;
  ...
  // hideEquipModal is already defined at the top of the script, no need to redefine
  
  // Make function globally accessible
  window.saveEquipmentAndContinue = async function saveEquipmentAndContinue(){
    ...
    try {
      const response = await fetch('/set_equipment', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
      ...
      await response.json();
      await updateEquipmentDisplay();
    } catch(e) {
      alert('Error saving equipment: ' + e.message);
    }
  }
  ...
  setInterval(() => {
    ...
    // Check camera status on page load and show helpful message if not connected
    setTimeout(async function() {
      try {
        const r = await fetch('/status');
        const j = await r.json();
        const statusEl = document.getElementById('cameraStatus');
        if (statusEl && (!j.video_running || j.status && j.status.startsWith('error'))) {
          statusEl.style.display = 'block';
          statusEl.style.background = '#7f1d1d';
          statusEl.style.color = '#fecaca';
          statusEl.textContent = '⚠ Camera not connected! Click Auto-Detect OBS or Connect to Camera to connect. Make sure OBS Studio is running and Virtual Camera is started.';
        }
      } catch(e) {
        console.error('Error checking camera status:', e);
      }
    }, 2000); // Wait 2 seconds after page load for auto-connect to complete
...
def peek():
    ...
    try:
        ...
        try:
            ...
            pass
# REMOVED: PowerShell camera enumeration functions
# These triggered antivirus detection. We now use simple direct OpenCV camera access only.

# Helper: Auto-detect OBS Virtual Camera or first available camera
def identify_camera_type(cap, idx):
    """
    Try to identify camera type based on properties and device names.
    ...
    try:
        ...
        if platform.system() == "Darwin":  # macOS
            ...
            # Windows: Use resolution-based detection (device name lookup removed to avoid antivirus)
            if platform.system() == "Windows":
                ...
                # Use resolution and index to identify OBS Virtual Camera
                # OBS Virtual Camera typically appears at indices 1-3 with high resolution
                if obs_running and idx in [1, 2, 3] and is_standard_resolution:
                    print(f"[identify_camera_type] Detected OBS Virtual Camera by resolution/index: {width}x{height} at index {idx}")
                    return "obs_virtual"
                
                # Fallback: Use resolution and OBS status if name detection failed
                if obs_running and is_standard_resolution:
                    ...
                    if idx >= 1:
                        ...
                        print(f"[identify_camera_type] Detected OBS Virtual Camera by resolution (fallback): {width}x{height} at index {idx}")
                        return "obs_virtual"  # Likely OBS Virtual Camera
                    elif idx == 0:
                        ...
                        if not is_small_resolution and (width >= 1280 or height >= 720):
                            print(f"[identify_camera_type] Detected OBS Virtual Camera at index 0 (fallback): {width}x{height}")
                            return "obs_virtual"  # Could be OBS Virtual Camera
                        else:
                            # Index 0 with small resolution when OBS is running = probably built-in webcam
                            # But OBS Virtual Camera might still be available at higher indices
                            print(f"[identify_camera_type] Index 0 is small resolution ({width}x{height}) - likely built-in webcam, not OBS")
                            return "other"  # Built-in webcam, not OBS
        ...
        print(f"[identify_camera_type] Error identifying camera: {e}")
        return "unknown"
...
def auto_detect_camera():
    """
    Auto-detect OBS Virtual Camera using simple, direct approach.
    Avoids PowerShell and complex enumeration that triggers antivirus.
    Returns: (source_index, backend_code, backend_name)
    """
    print("[Camera Detection] Starting simple OBS Virtual Camera detection...")
    
    if platform.system() == "Windows":
        # Simple approach: Check if OBS is running (minimal subprocess call)
        obs_running = False
        try:
            result = subprocess.run(['tasklist', '/FI', 'IMAGENAME eq obs64.exe'], 
                                  capture_output=True, timeout=1)
            obs_running = result.returncode == 0 and b'obs64.exe' in result.stdout
            if obs_running:
                print("[Camera Detection] ✓ OBS is running")
            else:
                print("[Camera Detection] ⚠ OBS is not running - will still try common indices")
        except Exception as e:
            print(f"[Camera Detection] Could not check OBS status: {e}, proceeding anyway")
        
        # SIMPLIFIED: Try indices 1, 2, 3 with DSHOW backend first, then MSMF as fallback
        # This is the approach that worked - no PowerShell, no complex enumeration
        backends_to_try = [
            ("DSHOW", cv2.CAP_DSHOW),
            ("MSMF", cv2.CAP_MSMF)
        ]
        
        # Try indices 1, 2, 3 first (OBS Virtual Camera typically appears here)
        # If OBS is running, prioritize these. If not, also try index 0
        indices_to_try = [1, 2, 3] if obs_running else [1, 2, 3, 0]
        
        for backend_name, backend_code in backends_to_try:
            print(f"[Camera Detection] Trying {backend_name} backend...")
            for idx in indices_to_try:
                try:
                    ...
                    if cap.isOpened():
                        # Set properties that help OBS Virtual Camera connect
                        try:
                            cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)
                            if idx in [1, 2, 3]:
                                cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1920)
                        ...
                        if ret and frame is not None:
                            ...
                            is_black = frame_mean < 10
                            
                            print(f"[Camera Detection] Camera at index {idx} ({backend_name}): size={w}x{h}, mean={frame_mean:.1f}, black={is_black}")
                            
                            # VERY LENIENT: If OBS is running and we find ANY camera at indices 1-3, accept it immediately
                            # OBS Virtual Camera can appear at various resolutions, so accept any working camera
                            if obs_running and idx in [1, 2, 3]:
                                print(f"[Camera Detection] ✓✓✓ ACCEPTED: Camera at index {idx} (OBS running, accepting as OBS Virtual Camera): size={w}x{h}, mean={frame_mean:.1f}")
                                if is_black:
                                    print(f"[Camera Detection] ⚠ Frame is black - make sure OBS Virtual Camera is started in OBS Studio")
                                else:
                                    print(f"[Camera Detection] ✓ Frame is not black - camera appears to be streaming")
                                camera_found = True
                                cap.release()
                                return idx, backend_code, backend_name
                            elif not obs_running and idx in [1, 2, 3]:
                                # Even if OBS not detected as running, indices 1-3 are likely OBS Virtual Camera
                                # Accept if it's high-res OR if it's any resolution (OBS might not be detected but still running)
                                if w >= 1280 and h >= 720:
                                    print(f"[Camera Detection] ✓ ACCEPTED: High-res camera at index {idx} (likely OBS): size={w}x{h}")
                                    camera_found = True
                                    cap.release()
                                    return idx, backend_code, backend_name
                                else:
                                    # Still accept it - might be OBS at different resolution
                                    print(f"[Camera Detection] ✓ ACCEPTED: Camera at index {idx} (possibly OBS): size={w}x{h}")
                                    camera_found = True
                                    cap.release()
                                    return idx, backend_code, backend_name
                            elif idx == 0 and w >= 1280 and h >= 720:
                                # High-res at index 0 could also be OBS Virtual Camera
                                print(f"[Camera Detection] ✓ ACCEPTED: High-res camera at index 0: size={w}x{h}")
                                camera_found = True
                                cap.release()
                                return idx, backend_code, backend_name
                            else:
                                print(f"[Camera Detection] ⚠ REJECTED: Camera at index {idx} doesn't match OBS Virtual Camera criteria (size={w}x{h}, OBS running={obs_running})")
                    ...
                    try:
                        ...
                        pass
                    print(f"[Camera Detection] Exception testing index {idx}: {e}")
                    continue
            
            # Only break if we found a camera (but we return immediately when found, so this shouldn't be reached)
            # If we get here, no camera found with this backend, try next backend
        
        # If we get here, no OBS Virtual Camera found - try index 1 anyway
        # This allows the app to work even if detection fails, user can manually adjust
        print("[Camera Detection] ⚠⚠⚠ NO CAMERA FOUND at indices 1, 2, 3 with either backend")
        print("[Camera Detection] ⚠ TROUBLESHOOTING:")
        print("[Camera Detection] ⚠   1. Make sure OBS Studio is running")
        print("[Camera Detection] ⚠   2. In OBS Studio, click 'Start Virtual Camera' button")
        print("[Camera Detection] ⚠   3. Make sure you have an active source/scene in OBS")
        print("[Camera Detection] ⚠   4. Try manually selecting Source 1, 2, or 3 in the Camera Source dropdown")
        print("[Camera Detection] ⚠   5. Try clicking 'Auto-Detect OBS' button")
        print("[Camera Detection] ⚠   6. Check Windows Device Manager to see if OBS Virtual Camera appears")
        print("[Camera Detection] ⚠ Using index 1 as fallback - will attempt to connect anyway")
        print("[Camera Detection] ⚠ If connection fails, manually try indices 0, 1, 2, 3 in the UI")
        return 1, cv2.CAP_DSHOW, "DSHOW"
        ...
        # SIMPLIFIED: Try indices 1, 2, 3 first (OBS Virtual Camera typically appears here)
        # If OBS is running, prioritize these. If not, also try index 0
        indices_to_try = [1, 2, 3] if obs_running else [1, 2, 3, 0]
        
        for idx in indices_to_try:
            try:
                print(f"[Camera Detection] Testing index {idx} with AVFOUNDATION...")
                cap = cv2.VideoCapture(idx, backend_code)
                ...
                if cap.isOpened():
                    ...
                    if ret and frame is not None:
                        ...
                        print(f"[Camera Detection] Camera at index {idx}: size={w}x{h}, mean={frame_mean:.1f}, black={is_black}")
        ...
        # If we get here, no OBS Virtual Camera found - try index 1 anyway
        # This allows the app to work even if detection fails, user can manually adjust
        print("[Camera Detection] ⚠⚠⚠ NO CAMERA FOUND at indices 1, 2, 3")
        print("[Camera Detection] ⚠ TROUBLESHOOTING:")
        print("[Camera Detection] ⚠   1. Make sure OBS Studio is running")
        print("[Camera Detection] ⚠   2. In OBS Studio, click 'Start Virtual Camera' button")
        print("[Camera Detection] ⚠   3. Make sure you have an active source/scene in OBS")
        ...
        print("[Camera Detection] ⚠ Using index 1 as fallback - will attempt to connect anyway")
        print("[Camera Detection] ⚠ If connection fails, manually try indices 0, 1, 2, 3 in the UI")
        return 1, backend_code, backend_name
...
def list_cameras():
    """List all available cameras with their types and device names"""
    try:
        cameras = []
        
        # Camera name enumeration removed to avoid antivirus triggers
        # We now use simple direct OpenCV access only
            ...
            for idx in range(max_scan + 1):
                try:
                    ...
                    if cap.isOpened():
                        ...
                        # Try to read frame with timeout (Windows can hang)
                        frame_result = [None, None]
                        frame_ready = threading.Event()
                        ...
                        if frame_ready.is_set():
                            ...
                            if ret and frame is not None:
                                ...
                                # Device name lookup removed to avoid antivirus triggers
...
def system_cameras():
    ...
    try:
        ...
        if platform.system() == "Windows":
            # Windows: PowerShell camera enumeration removed to avoid antivirus triggers
            # We now use only direct OpenCV camera access
            cameras_info.append({
                'method': 'Windows Camera Detection',
                'note': 'PowerShell enumeration removed for security. Use "List Cameras" button to see available cameras via OpenCV.'
            })
        ...
        if not mon.video_running:
            ...
            try:
                ...
                if not mon.video_running or mon.status in ["error", "error: cannot read from source", "error: cannot open video source"]:
                    # Even if video fails, still create CSV so logging can start when camera connects
                    print(f"[Start Test] Video failed to start (status: {mon.status}), but creating CSV anyway")
                    print(f"[Start Test] User can connect camera later and logging will work")
                    # Continue to create CSV below
            except Exception as e:
...
if __name__ == "__main__":
    ...
    def auto_connect_camera():
        ...
        try:
            ...
            # Check if OBS is running first to prioritize OBS Virtual Camera (both Windows and Mac)
            obs_running = False
            if platform.system() == "Windows":
                try:
                    result = subprocess.run(['tasklist', '/FI', 'IMAGENAME eq obs64.exe'], 
            ...
            # Add timeout wrapper for auto_detect_camera to prevent hanging (both Windows and Mac)
            detected_idx = None
            detected_backend_code = None
            detected_backend_name = None
            detection_result = [None, None, None]
            detection_done = threading.Event()
            
            def detect_with_timeout():
                try:
                    idx, backend_code, backend_name = auto_detect_camera()
                    detection_result[0] = idx
                    detection_result[1] = backend_code
                    detection_result[2] = backend_name
                except Exception as e:
                    print(f"[Startup] Error in auto_detect_camera: {e}")
                    import traceback
                    traceback.print_exc()
                finally:
                    detection_done.set()
            
            # Run detection in a separate thread with timeout
            detect_thread = threading.Thread(target=detect_with_timeout, daemon=True)
            detect_thread.start()
            
            # Wait up to 15 seconds for detection (Windows can be slow, Mac usually faster)
            timeout_seconds = 15.0 if platform.system() == "Windows" else 10.0
            if detection_done.wait(timeout=timeout_seconds):
                detected_idx, detected_backend_code, detected_backend_name = detection_result[0], detection_result[1], detection_result[2]
            else:
                print(f"[Startup] ⚠ Camera detection timed out after {timeout_seconds} seconds!")
                # Use platform-appropriate defaults
                if platform.system() == "Windows":
                    print("[Startup] Using default camera settings (Source=1, Backend=DSHOW)")
                    detected_idx = 1
                    detected_backend_code = cv2.CAP_DSHOW
                    detected_backend_name = "DSHOW"
                elif platform.system() == "Darwin":
                    print("[Startup] Using default camera settings (Source=1, Backend=AVFOUNDATION)")
                    detected_idx = 1
                    detected_backend_code = cv2.CAP_AVFOUNDATION
                    detected_backend_name = "AVFOUNDATION"
                else:
                    detected_idx = 0
                    detected_backend_code = cv2.CAP_V4L2
                    detected_backend_name = "V4L2"
            
            if detected_idx is None:
                print("[Startup] ⚠ Camera detection returned None, using defaults")
                # Use platform-appropriate defaults
                if platform.system() == "Windows":
                    detected_idx = 1
                    detected_backend_code = cv2.CAP_DSHOW
            ...
            with mon.lock:
                if not mon.video_running:  # Only start if not already running
                    ...
                    # Give video thread a moment to initialize (Mac usually faster than Windows)
                    sleep_time = 2.0 if platform.system() == "Windows" else 1.0
                    ...
                    with mon.lock:
                        if mon.video_running and mon.status not in ["error", "error: cannot read from source", "error: cannot open video source"]:
                            ...
                            if mon.status.startswith("error"):
                                print(f"[Startup] ⚠ Error status: {mon.status}")
                else:
            ...
            import traceback
            traceback.print_exc()
            # Set defaults if auto-connect fails (platform-appropriate)
            with mon.lock:
                if not mon.video_running:
                    ...
                    try:
                        if platform.system() == "Windows":
                            ...
                            mon.src = 1
                            mon.backend_name = "AVFOUNDATION"
                        else:
                            mon.src = 0
                            mon.backend_name = "V4L2"
                        mon.video_running = True
                        mon.running = True
                        mon.status = "video_starting"
                        mon.last_frame = None
                        ...
                        mon.worker = t
                        t.start()
                        print(f"[Startup] Default video feed thread started (Source={mon.src}, {mon.backend_name})")
